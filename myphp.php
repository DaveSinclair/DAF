        ...
        <div dojoType="dijit.MenuBar" id="navMenu" style="float:left; display:none">
            ...
            <div dojoType="dijit.PopupMenuBarItem">
                ...
                <div dojoType="dijit.Menu" id="adminMenu" style="display:none">
                    ...
                    <div dojoType="dijit.MenuItem" onclick="window.location='index.php?object=version&action=show'">
                        Show Version
                    </div>
                </div>
            </div>
...
function selectorENUMScenariostatus($index) {           /* <<<<<<<< shouldn't this be determined automatically from a query ?  */
...
//---------------------------------------------------------------------------------------------------------------------
//
// Function:   DAF_web_interface_version  
//
// Inputs:     none
//           
// Outputs:    none                                                                                                 
// 
// Returns:    A text string indicating the version and build time of this DAF web interface installation,                              
//
// Description:  
//
// The make_daf_package will change the XXBUILD_DATEXX field below so that this routine returns a string like
//
//   "DAF web interface version 1.2 built on 21 March 2011 at 09:30:21"
//
//----------------------------------------------------------------------------------------------------------------------

function DAF_web_interface_version() {

   $version_string = "DAF web interface version 1.1 built on XXBUILD_DATEXX";
...
//---------------------------------------------------------------------------------------------------------------------
//
// Function:   DAF_server_version  
//
// Inputs:     none
//           
// Outputs:    none                                                                                                 
// 
// Returns:    A text string indicating the version and build time of this DAF server installation,                              
//
// Description:  
//
// A returns a string like daf 1.0: (compiled on Jan  5 2012 at 20:53:59)
//
//----------------------------------------------------------------------------------------------------------------------

function DAF_server_version() {

   // we expect to see something like: daf 1.0: (compiled on Jan  5 2012 at 20:53:59) from the daf server
   
   $version_string = "unknown";
   
   exec("/opt/daf/daf 2>&1", $output, $rc);

   if ($rc == 0) {
      foreach ($output as $line) {
           $num_matches = preg_match("/compiled on/", $line, $matches);
         if ($num_matches > 0) {
           $version_string = $line;
         }
      }
