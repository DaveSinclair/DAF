...
#define TIMER_INIT_VALUE 1000000 
...
Iu32 get_timestamp( Iu32 *sec, Iu32 *msec, Iu32 *usec ) {
 ...
 timer_us = ((TIMER_INIT_VALUE - time_now.it_value.tv_sec) * 1000000
                    - (time_now.it_value.tv_usec ));

 *usec = timer_us % 1000;
 *msec = (timer_us / 1000) % 1000;
 *sec  = timer_us / 1000000;
...
#define MAX_NUM_ENVIRONMENT_VALUES 100
   ...
   if (! find_cmd_log(&p_service_cmd_log_object, msg, sizeof(msg))) {
      ...
      if (args->run_in_shell ) {
         ...
         snprintf(msg, sizeof(msg), "Executing in shell: tag=%u\n", args->tag);
         log_msg_to_scenario_console_log(agent_log_pathname, msg, args->cmdstring, FALSE);
         ...
         log_msg_to_scenario_console_log(agent_log_pathname, prefix, msg, TRUE); 
            ...
            i = 0; cmdstring_length = strlen(args->cmdstring);
            ...
            make_host_and_time_specific_pathname_prefix(DAF_SERVICE_LOG_DIR, ThisHostName, args->actionresultID, testcasename, "stdout",
                                                        stdout_pathname, sizeof(stdout_pathname));
            ...
            make_host_and_time_specific_pathname_prefix(DAF_SERVICE_LOG_DIR, ThisHostName, args->actionresultID, testcasename, "stderr",   
                                                        stderr_pathname, sizeof(stderr_pathname));
            ...
            //close(fd_stdout); /* close the file descriptor as we don't need it more  */
            //close(fd_stdout); /* close the file descriptor as we don't need it more  */
...
#define SUBNAME "client_remote_client_cmd_1_svc"
   ...
   if (! args->run_in_background) {
     ...
     if ((pid = fork()) == 0) {
      ...
      if (update_cmd_log_entry(p_service_cmd_log_object, "", tag, CMD_COMPLETED, rc, FALSE, 0) != 0) {       /* <<< do we even need the waitpid in reaper */   printf("error 3\n");
      }
...
#define SUBNAME "zombie_reaper"
    ...
    while (1) {
      ...
      for (i=0; i< DAF_MAX_COMMANDS; i++ ) {
         ...
         if (p_service_cmd_log_object->cmd_array[i].state == CMD_CANCELED_TIMELIMIT) {
         ...
         } else if (p_service_cmd_log_object->cmd_array[i].state == CMD_CANCELED_TIMEDOUT) {
            ...
            if (rc > 0) {
               ...
               /* -----------------------------------------------------------------------------------------------------------------*/
               /* command was deliberatily canceled with signal 9 as it overran - allow this bad status to be reported to indicate */
               /* that the command failed                                                                                          */
               /* -----------------------------------------------------------------------------------------------------------------*/

               p_service_cmd_log_object->cmd_array[i].state    = CMD_COMPLETED_TIMEDOUT;
