...
function create_phase_database($project, $phase) {
  ...
  PRIMARY KEY (`ID`)
   ...
   $query = "CREATE TABLE $status_db.`testlevelrecordmember` (
     `ID` int(11) NOT NULL AUTO_INCREMENT,
     `TestlevelrecordID` int(11) NOT NULL,
     `Levelname` varchar(32) NOT NULL,
     `Objecttype` varchar(32) NOT NULL,
     `Levelinstancevalue` varchar(256) NOT NULL,
     `State` enum('unknown','proxy_collectorhost_no_connect','proxy_collectorhost_fail','match','no_match') NOT NULL,
     PRIMARY KEY (`ID`)
   ) ENGINE=InnoDB  DEFAULT CHARSET=latin1 AUTO_INCREMENT=1 ;";
   ...
   $query = "CREATE TABLE $status_db.`teststandrecordmember` (
     `ID` int(11) NOT NULL AUTO_INCREMENT,
     `TeststandrecordID` int(11) NOT NULL,
     `Objecttype` varchar(32) NOT NULL,
   ...
   $query = "CREATE TABLE $status_db.`teststandrecordmemberattribute` (
     `ID` int(11) NOT NULL AUTO_INCREMENT,
     `TeststandrecordmemberID` int(11) NOT NULL,
...
_END;
   ...
   echo "<input type=\"hidden\" name=\"object\" value=\"outcomeactioon\"/>\n";
...
function dafserver($gp) {
   ...
   //  show_dafserver($gp, $completion_message);
      ...
      show_path("/opt/daf/logs/suse11a_console.log");                  /* <<<<<<<<<< what should this really be?  */
      ...
      show_path("/opt/lampp/var/mysql/suse11a.err");                  /* <<<<<<<<<< what should this really be?  */
...
function show_all_auditlogs1($status_db, $auditlog_startviewdate, $auditlog_endviewdate,
                             $auditlog_startviewtime, $auditlog_endviewtime, $auditlog_Ndays) {
...
echo <<<_END
         ...
         var grid = new dojox.grid.DataGrid({    // use the adapter to give the grid a Dojo Data compliant interface to the store    
                            ...
                            structure: [ {name:"ID", field:"ID", width:"3em", styles:"text-align:center;border-width: 1px; border-color:red;"},        
		                                 ...
		                                 {name:"Content", field:"Content", width:"80em"}
		                               ]}, 
		                    "AuditlogStatus");
_END;
         echo "grid.setQuery({Date: \"[" .  $auditlog_startviewdate . " " . $auditlog_startviewtime . "," . 
                                            $auditlog_endviewdate . " " . $auditlog_endviewtime . "]\"});";
echo <<<_END
		 grid.startup();
...
// Description:
//
// Handles requests to do with version info.                                        
//
//----------------------------------------------------------------------------------------------------------------------

function version($gp) {
  
   if ($gp['object'] !== "version") {
      add_error_message("DAFE0001: version: internal error - bad object - " . $gp['object']);
      $gp['action'] = 'show'; $gp['ID'] = NULL; $gp['object'] = "version";
   }
...
// Description:
//
// Displays the version information for the DAF web interface and DAF server 
//
//--------------------------------------------------------------------------------------------------------------------
...
function show_version_info() {
   ...
   echo '<table class="tab1">';
   echo "<caption class=\"cap1\"><div>Version Information</div></caption>\n";
   echo "<tr><th>Web interface version</th><td>" . DAF_web_interface_version() . "</td></tr>\n";
   echo "<tr><th>DAF server version</th><td>" . DAF_server_version() . "</td></tr>\n";
   echo '</table>';
