...
xdr_remote_client_cmd_args (XDR *xdrs, remote_client_cmd_args *objp)
{
	register int32_t *buf;
	 ...
	 if (!xdr_Iu32 (xdrs, &objp->actionresultID))
		 return FALSE;
	 if (!xdr_string (xdrs, &objp->sql_servername, MAX_SQLSERVERNAME_LEN))
		 return FALSE;
	 if (!xdr_string (xdrs, &objp->sql_username, MAX_SQLUSERNAME_LEN))
		 return FALSE;
	 if (!xdr_string (xdrs, &objp->sql_password, MAX_SQLPASSWORD_LEN))
		 return FALSE;
	 if (!xdr_string (xdrs, &objp->sql_databasename, MAX_SQLDATABASENAME_LEN))
		 return FALSE;
	 if (!xdr_short (xdrs, &objp->sql_port))
		 return FALSE;
	 if (!xdr_int (xdrs, &objp->msglevel))
		 return FALSE;
	return TRUE;
