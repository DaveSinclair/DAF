...
function scenarioresult($gp, $current_project, $current_phase, $scenarioresult_startviewdate, $scenarioresult_endviewdate, 
   ...
   if ($current_project == "none" || $current_phase == "none") {
     ...
     add_error_message("DAFE0055: Cannot show scenario results for project $current_project phase $current_phase");
     return; 
   }
...
function show_all_scenarioresults1($status_db, $scenarioresult_startviewdate, $scenarioresult_endviewdate, 
 ...
 echo '<div style="border-width: 1px; border-color:red;">';
...
echo <<<_END
   ...
   <div id="ScenarioresultStatus" style="width:1024px; height:650px;"></div>
   <script type="text/javascript">
      dojo.require("dojo.parser");
      dojo.require("dojox.grid.DataGrid");
      dojo.require("dojo.data.ObjectStore");
      
      function formatJobname(val, rowId) {      
        var ID=this.grid.store.getValue(this.grid.getItem(rowId), 'ID');
        return '<a href=index.php?action=show&object=scenarioresult&ID=' + ID + '>' +  val + '</a>';
      }
      
      function formatTeststandrecord(val, rowId) {      
        var ID=this.grid.store.getValue(this.grid.getItem(rowId), 'TeststandrecordID');
        return '<a href=index.php?action=show&object=teststandrecord&ID=' + ID + '>' +  val + '</a>';
      }
      
      function formatTestlevelrecord(val, rowId) {      
        var ID=this.grid.store.getValue(this.grid.getItem(rowId), 'TestlevelrecordID');
        return '<a href=index.php?action=show&object=testlevelrecord&ID=' + ID + '>' +  val + '</a>';
      }
      
      function formatLoglocation(val, rowId) {      
        var dir=this.grid.store.getValue(this.grid.getItem(rowId), 'Loglocation');
        return '<a href=index.php?action=show&object=directory&dir=' + dir + '>' +  val + '</a>';
      }

      function formatScenariologfilename(val, rowId) {      
        var dir=this.grid.store.getValue(this.grid.getItem(rowId), 'Loglocation');
        var logfile=this.grid.store.getValue(this.grid.getItem(rowId), 'Scenariologfilename');
        return '<a href=index.php?action=show&object=path&path=' + dir + '/' + val + '>' +  val + '</a>';
      }
...
echo <<<_END
         ...
         var grid = new dojox.grid.DataGrid({        
                            ...
                            structure: [ {name:"ID", field:"ID", width:"3em", styles:"text-align:center;"},        
		                                 ...
		                                 {name:"Job Name", field:"Jobname", width:"8em", formatter: formatJobname},        
		                                 ...
		                                 {name:"Actions Completed", field:"Actionscompleted", width:"6em", styles:"text-align:center;"},  
		                                 ...
		                                 {name:"Test Stand", field:"Teststand", width:"8em", formatter: formatTeststandrecord},
		                                 {name:"Test Level", field:"Testlevel", width:"8em", formatter: formatTestlevelrecord},
		                                 {name:"Tester", field:"Tester", width:"8em"},
		                                 {name:"Log Location", field:"Loglocation", width:"12em", formatter: formatLoglocation},
		                                 {name:"Scenario Log Filename", field:"Scenariologfilename", width:"8em", formatter: formatScenariologfilename},
		                                 {name:"Comments", field:"Comments", width:"8em"}
		                               ]}, 
		                    "ScenarioresultStatus"); 

_END;
         echo "grid.setQuery({Start: \"[" .  $scenarioresult_startviewdate . " " . $scenarioresult_startviewtime . "," . 
                                             $scenarioresult_endviewdate . " " . $scenarioresult_endviewtime . "]\"});";
echo <<<_END
...
function testlevelrecord($gp, $current_project, $current_phase) {
  
   global $db_server;

   if ($gp['object'] !== "testlevelrecord") {
      add_error_message("DAFE0001: testlevelrecord: internal error - bad object - " . $gp['object']);
     $gp['action'] = 'show'; $gp['ID'] = NULL; $gp['object'] = "testlevelrecord";
   }
   
   if ($current_project == "none" || $current_phase == "none") {
   
     add_error_message("DAFE0099: Cannot show testlevelrecord results for project $current_project phase $current_phase");
     return; 
   }
   ...
   if ($gp['action'] === "show") {
      ...
      if (isset($gp['ID'])) {
          ...
          show_detailed_testlevelrecord($status_db, $gp['ID']);
      
      } else {

          show_all_testlevelrecords($status_db);
      
      }
   
   } else {
    
      report_bad_parms("testlevelrecord");
...
//---------------------------------------------------------------------------------------------------------------------
//
// Function:   show_all_testlevelrecords
//
// Inputs:     $status_db         The name of the status database for the project/phase being considered
//             $ID
//
...
// Description:
//
//  Displays a table of testlevelrecords
// 
//----------------------------------------------------------------------------------------------------------------------

function show_detailed_testlevelrecord($status_db, $ID) {
   ...
   echo '<form style="float:left" action="index.php" method="post">' . "\n";
   echo '<table class="tab1">';
   echo "<caption class=\"cap1\"><div>TestLevelRecord  ID = $ID</div></caption>";
   echo "\n";
   ...
   $tablename = determine_status_table_name($status_db, "testlevelrecordmember");
   
   $query = "SELECT * FROM " . $tablename . " WHERE testlevelrecordID = '" . $ID . "'";
   ...
   echo '<table class="tab1">';
   echo "<caption class=\"cap1\"><div>TestLevelRecord Members</div></caption>";
   echo "\n";
   ...
   for ($i = 0; $i < $num_fields; $i++) {
       ...
       if ($fieldname == 'ID') {
          ...
          echo '<th>Test Level Record ID</th>';
       } else if ($fieldname == 'Levelname') {
          echo '<th>Level Name</th>';
       } else  if ($fieldname == 'Objecttype') {
          echo '<th>Object Type</th>';
       } else  if ($fieldname == 'Levelinstancevalue') {
...
function show_all_testlevelrecords($status_db) {
   ...
   $num_rows = mysql_num_rows($result);             /* shouldnt we check for number of rows <> 1  ...and have error handling code, here and in other routines? */
   $num_fields = mysql_num_fields($result);
   $row = mysql_fetch_row($result);

   echo '<table class="tab1">';
   echo "<caption class=\"cap1\"><div>TestLevelRecord Results</div></caption>";
   echo "\n";
   ...
   for ($i = 0; $i < $num_fields; $i++) {
       ...
       if ($fieldname == 'ID') {
          echo '<th>ID</th>';
          echo '<td>' . $row[$i] . '</td>';
       } else if ($fieldname == 'ScenarioresultID') {
          $scenarioresultID = $row[$i];
          echo '<th>Scenario Result ID</th>';
          echo '<td>' . $row[$i] . '</td>';
       } else  if ($fieldname == 'Name') {
          echo '<th>Test Level Name</th>';
          echo '<td>' . $row[$i] . '</td>';
       } else {
          echo '<th>' . $fieldname . '</th>';
          echo '<td>' . $row[$i] . '</td>';
...
function teststandrecord($gp, $current_project, $current_phase) {
  
   global $db_server;

   if ($gp['object'] !== "teststandrecord") {
      add_error_message("DAFE0001: teststandrecord: internal error - bad object - " . $gp['object']);
      $gp['action'] = 'show'; $gp['ID'] = NULL; $gp['object'] = "teststandrecord";
   }
   
   if ($current_project == "none" || $current_phase == "none") {
   
     add_error_message("DAFE0099: Cannot show teststandrecord results for project $current_project phase $current_phase");
     return; 
   }
   ...
   if ($gp['action'] === "show") {
      ...
      if (isset($gp['ID'])) {
          ...
          show_detailed_teststandrecord($status_db, $gp['ID']);
      
      } else {

          show_all_teststandrecords($status_db);
      
      }
   
   } else {
    
      report_bad_parms("teststandrecord");
...
//---------------------------------------------------------------------------------------------------------------------
//
// Function:   show_all_teststandrecords
//
// Inputs:     $status_db         The name of the status database for the project/phase being considered
//             $ID
//
...
// Description:
//
//  Displays a table of teststandrecords
// 
//----------------------------------------------------------------------------------------------------------------------

function show_detailed_teststandrecord($status_db, $ID) {
   ...
   echo '<form style="float:left" action="index.php" method="post">' . "\n";
   echo '<table class="tab1">';
   echo "<caption class=\"cap1\"><div>Test Stand Record ID = $ID</div></caption>";
   echo "\n";
   ...
   $tablename = determine_status_table_name($status_db, "teststandrecordmember");
   
   $query = "SELECT * FROM " . $tablename . " WHERE teststandrecordID = '" . $ID . "'";
   ...
   echo '<table class="tab1">';
   echo "<caption class=\"cap1\"><div>TestStandRecord Members</div></caption>";
   echo "\n";
   ...
   for ($i = 0; $i < $num_fields; $i++) {
       ...
       if ($fieldname == 'ID') {
          ...
          echo '<th>Test Stand Record ID</th>';
       } else  if ($fieldname == 'Objecttype') {
          echo '<th>Object Type</th>';
       } else  if ($fieldname == 'Objectname') {
          echo '<th>Objectname</th>';
       } else {
          echo '<th>' . $fieldname . '</th>';
       }
   }
...
function show_all_teststandrecords($status_db) {
   ...
   echo '<table class="tab1">';
   echo "<caption class=\"cap1\"><div>TestStandRecord Results</div></caption>";
   echo "\n";
