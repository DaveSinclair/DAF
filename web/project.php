...
_END;
   ...
   echo '<table class="tab1">';
   echo "<caption class=\"cap1\"><div>Modify Scenario Outcome Action</div></caption>";
   echo "\n";
...
function show_detailed_maillist($maillistID) {
   ...
   for ($i=0; $i<$num_rows; $i++) {
      ...
      button_with_confirm("maillist_delete_$i", "delete", "delete", "index.php?action=show&operation=deletemaillistmember&object=maillist&ID=$maillistmemberID&maillistID=$maillistID",
...
function dafserver($gp) {
   ...
   //  show_dafserver($gp, $completion_message);
      ...
      $hostname = trim(`hostname -s`);
      show_path("/opt/daf/logs/" . $hostname . "_console.log");                  /* <<<<<<<<<< what should this really be?  */
      ...
      $hostname = trim(`hostname -s`);
      show_path("/opt/lampp/var/mysql/" . $hostname . ".err");                  /* <<<<<<<<<< what should this really be?  */
...
echo <<<_END
      ...
      <tr><th>Phase</th>
      <td id="phaseselectortdID"><select id="backupPhaseId" name="phaseID"/>
      <--- onchange on the project select will fill this in --->
_END;
...
echo <<<_END
   ...
   <input type="hidden" name=object value="mainmenu" />
