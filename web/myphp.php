...
function head() {
  ...
  <head>
		...
		<!-- load bootstrap, jquery, jquery form -->
	    ...
	    <script src="js/daf.js" type="text/javascript"></script>
...
//
// ----------------------------------------------------------------------------------------------------------------------
function show_table($tablename, $title, $where = NULL, $orderby = NULL, $show = NULL, $modify = NULL, $delete = NULL, $links = array()) {
...
function selectorIDforTestLevelInstance($ID = NULL) {
	$query = "SELECT levelinstance.ID, level.name, daf.levelinstance.value FROM daf.levelinstance INNER JOIN daf.level " . "ON daf.levelinstance.levelID = daf.level.ID ORDER BY daf.level.name";
	$result = mysql_query ( $query );
...
function selectoractionstatus($value = 'xxxx') { /* <<<<<<<< shouldn't this be determined automatically from a query ? */
	$a = array (
			"None",
			"Canceled_due_to_error_in_step",
			"Completed",
			"No_available_host",
			"No_remote_host_connect",
			"Clnt_call_failed",
			"Prepare_cmd_failed",
			"Execute_cmd_failed" 
	);
...
function determine_objecttype_attributes($objecttype) {
	...
	$query = "SELECT objectattributetype.Name FROM daf.objectattributetype INNER JOIN daf.objecttype ON objectattributetype.objecttypeID = objecttype.ID  " . "WHERE objecttype.Name = '" . $objecttype . "'";
...
function determine_objecttype_attribute_names_and_ids($objecttype) {
	...
	$query = "SELECT objectattributetype.ID, objectattributetype.Name FROM daf.objectattributetype INNER JOIN daf.objecttype ON objectattributetype.objecttypeID = objecttype.ID  " . "WHERE objecttype.Name = '" . $objecttype . "'";
...
function get_enum_fields($database, $tablename, $columnname) {
	$query = "SELECT COLUMN_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = '" . $database . "' AND TABLE_NAME = '" . $tablename . "' AND COLUMN_NAME = '" . $columnname . "'";
...
function button_with_confirm($id, $label, $verb, $gowindowlocation, $nogowindowlocation) {
	echo "   <button id=\"$id\" data-dojo-type=\"dijit.form.Button\" data-dojo-props=\"type:'button', " . "onClick:function(){ if (window.confirm('Are you sure you want to " . $verb . " this item?')) {window.location = '$gowindowlocation'} else {window.location = '$nogowindowlocation'} } \">\n";
	echo "       $label\n";
	echo "   </button>\n";
}
